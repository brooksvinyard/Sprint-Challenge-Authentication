...
1. What is the purpose of using _sessions_?
    ...
    One can use sessions to keep track of a logged in user, otherwise a user would have to login on every page refresh.

1. What does bcrypt do to help us store passwords in a secure manner.

    `bcrypt` will salt and encrypt the user's password for us very securely

1. What does bcrypt do to slow down attackers?

    `bcrypt` will salt the password, helping to prevent rainbow table lookups. `bcrypt` will also use many rounds to hash the password, slowing down an attacker.
